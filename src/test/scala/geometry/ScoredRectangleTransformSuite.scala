...
class ScoredRectangleTransformSuite extends AnyFunSuite {
  ...
  test("scale all by 3 should yield rectangles with triple the width and height") {
    val scaled = scaleAll(3, sampleRectangles)
    val expected = ArraySeq(
      Rectangle(0, 0, 15, 12),
      Rectangle(10, 5, 6, 9),
      Rectangle(-9.5, 2, 12, 1.5),
      Rectangle(-23.8, -19.6, 51, 36),
      Rectangle(23, 25, 30, 39),
    )
    assert(scaled == expected)
  }
  ...
  test("removeEmpty with only non-empty rectangles should yield the same set again") {
    val emptyRemoved = removeEmpty(sampleRectangles)

    assert(emptyRemoved.length == sampleRectangles.length)

    assert(emptyRemoved.zip(sampleRectangles).forall(t => t._1 === t._2))
  }
  ...
  test("removeEmpty with a set containing a single empty rectangle should yield the set without this rectangle") {
    ...
    assert(emptyRemoved.length == expected.length)
  ...
  test("findOverlappingRectangle with a square rectangle with length 1 and offset (1, 1) should return the first sample rectangle") {
  ...
  test("scaleToUnitArea should return rectangles with an area of 1") {
    val input = ArraySeq(Rectangle(0, 0, 3, 4))
    val output = scaleToUnitArea(input)
    assert(output.head.area === 1.0)
  }
  ...
  test("totalCircumference with an empty set should return 0") {
    assert(totalCircumference(ArraySeq()) == 0.0)
  }
  ...
  test("smallestScaleFactorToContain should return the smallest scala factor sufficient to make the first rectangle contain the second") {
    ...
    assert(smallestScaleFactorToContain(outer, inner, scaleFactors).contains(4.0))
  }

  test("smallestScaleFactorToContain should return the smallest scala factor sufficient to make the first rectangle contain the second even if scale factors aren't sorted") {
    val scaleFactors = ArraySeq(2.5, 1.5, 6.5, 4.5, 3.0, 8.5)
    val inner = Rectangle(0, 0, 7, 7)
    val outer = Rectangle(0, 0, 2, 2)
    assert(smallestScaleFactorToContain(outer, inner, scaleFactors).contains(4.5))
  }
